    ...
    for (int i = 0; i < brickLength; i++) {
        if (IsCollision(*obj, brick[i])) {
            ...
            if ((brick[i].cType == '?') && (obj->vertSpeed < 0) && (obj == &mario)) {
                brick[i].cType = '-';
                TObject* newCoin = GetNewMoving();
                InitObject(newCoin, brick[i].x, brick[i].y - 3, 3, 2, '$');
                newCoin->vertSpeed = -0.7;
            }
            if (brick[i].cType == '+') {
                level++;
                if (level > maxLvl) {
...
TObject* GetNewBrick() {
    brickLength++;
    TObject* temp = (TObject*)realloc(brick, sizeof(TObject) * brickLength);
...
TObject* GetNewMoving() {
    movingLength++;
    TObject* temp = (TObject*)realloc(moving, sizeof(TObject) * movingLength);
    if (!temp) exit(EXIT_FAILURE);
    moving = temp;
...
void CreateLevel(int lvl) {
    system("color 9F");
    free(brick); brick = nullptr; brickLength = 0;
    free(moving); moving = nullptr; movingLength = 0;
